...
module FoodMenus
  ...
  class Import
    ...
    def self.import_ingredients(user_id, file_name)
      ...
      user.shopping_list.try(:destroy)
      ...
      CSV.open(output_file, "w", headers: headings, write_headers: true) do |csv|
        file_data.each do |row|
          ...
          recipe = user.recipes.find_by(name: recipe_name) || FoodMenus::Recipe.create(name: recipe_name, user: user)

          ingredient = user.ingredients.find_by(name: ingredient_name) || FoodMenus::Ingredient.create(name: ingredient_name, user: user)
